...
func (g *GitClient) Init(branch *string) error {
	...
	if err := g.command("git", "config", "--global", "credential.helper", fmt.Sprintf("!f() { echo \"username=x-access-token\"; echo \"password=%s\"; }; f", g.AccessToken)).Run(); err != nil {
		return fmt.Errorf("failed to configure credential.helper")
	}
